...
Allmeans<- function() {
  ...
  terminalReach<- means(terminal_reaches)
  terminalReach$Experiment<- "terminalR"
  terminalProp<- means(terminal_localization)
  terminalProp$Experiment<- "terminalL"
  exposureReach<- emeans(exposure_reaches)
  exposureReach$Experiment<- "exposureR"
  exposureProp<- means(exposure_localization)
  exposureProp$Experiment<- "exposureL"
  
  Allmeans<- rbind(PassiveReach, PassiveProp, terminalReach, terminalProp, exposureReach, exposureProp)
  write.csv(Allmeans, 'ana/Descriptive Stat.csv', quote = FALSE, row.names = FALSE )
...
plotavereapoints<- function(){
  ...
  segments(x0 = samp2, y0 = Ttestdata$R1_Early[Ttestdata$Experiment == 'No-Cursor_No-Cursors_A']*-1, x1 = samp3, y1 = Ttestdata$R1_Late[Ttestdata$Experiment == 'No-Cursor_No-Cursors_A']*-1, col = rgb(0.0, 0.7, 0.0, 0.1))
